...
# ==========================================
# 0. 頁面配置與自訂 CSS (Mobile-First 競賽感風格)
# ==========================================
st.set_page_config(
    page_title="馬光家庭日｜院所搶位大挑戰",
    page_icon="🏆",
    layout="wide",
...
<style>
    ...
    .seat-available { background-color: #22c55e; color: white; border: 2px solid #16a34a; }
    .seat-taken { background-color: #ef4444; color: white; border: 2px solid #dc2626; opacity: 0.9; }
    .stButton>button {
        width: 100%;
        border-radius: 8px;
        height: 3em;
    ...
    if "db_initialized" not in st.session_state:
        ...
        st.session_state.questions = {
            "family_day": [
                {
                    "id": "F01",
                    "q": "今年馬光家庭日預計舉辦的主要核心主軸是什麼？",
                    "options": ["A. 極限體能挑戰", "B. 聚心同行・榮耀共好", "C. 閉門研討會", "D. 自由解散野餐"],
                    "ans": 1,
                    "exp": "今年家庭日主軸為『聚心同行・榮耀共好』，凝聚全員與眷屬向心力！"
                },
                {
                    "id": "F02",
                    "q": "家庭日當天報到時，員工需要出示什麼完成眷屬禮品領取？",
                    "options": ["A. 實體身分證", "B. 員工識別 QR Code", "C. 院所紙本名冊", "D. 健保卡"],
                    "ans": 1,
                    "exp": "請事先下載並出示活動專屬 QR Code，加速報到作業。"
                }
            ],
            "ma_kwang": [
                {
                    "id": "M01",
                    "q": "馬光醫療體系始終堅持的核心價值觀不包含下列何者？",
                    "options": ["A. 專業誠信", "B. 視病猶親", "C. 利潤至上", "D. 團隊協作"],
                    "ans": 2,
                    "exp": "馬光始終以患者與同仁福祉為第一優先，堅持專業醫療與溫暖關懷。"
                },
                {
                    "id": "M02",
                    "q": "馬光中醫推動的『一人一診室』主要目的是？",
                    "options": ["A. 增加裝潢成本", "B. 保護患者隱私與高規格就醫品質", "C. 方便醫師休息", "D. 配合消防法規"],
                    "ans": 1,
                    "exp": "提供患者安心且獨立的隱私看診環境，是馬光深耕醫療品質的承諾。"
                }
            ],
            "policy": [
                {
                    "id": "P01",
                    "q": "依據近期發布之院所內部滿意度指引，若同仁遇異常通報應於多久內登錄系統？",
                    "options": ["A. 24小時內", "B. 3天內", "C. 一週內", "D. 月底結算即可"],
                    "ans": 0,
                    "exp": "重大異常或客訴事件需於 24 小時內完成初步系統通報，以利跨部門即時協助。"
                },
                {
                    "id": "P02",
                    "q": "關於新修訂之同仁教育訓練學分補助政策，每年度最高可申請幾次外部進修？",
                    "options": ["A. 1次", "B. 2次", "C. 3次", "D. 依院長專案核定，原則每年2次"],
                    "ans": 3,
                    "exp": "政策鼓勵同仁持續精進，常規每年補助 2 次，特殊專案另行評估。"
                }
            ]
        }
        ...
        st.session_state.seats = {
            "A區": {"name": "A區 (近舞台特區)", "capacity": 100, "status": "available", "taken_by": None},
            "B區": {"name": "B區 (主中央景觀區)", "capacity": 80, "status": "available", "taken_by": None},
            "C區": {"name": "C區 (鄰近餐飲動線區)", "capacity": 120, "status": "available", "taken_by": None},
            "D區": {"name": "D區 (東側舒適沙發區)", "capacity": 60, "status": "available", "taken_by": None},
            "E區": {"name": "E區 (西側歡樂家庭區)", "capacity": 90, "status": "available", "taken_by": None},
        }
...
def render_live_leaderboard():
    st.subheader("🔥 院所搶位即時戰況 (60% 取得選位權)")
        ...
        with st.container():
            ...
            with col1:
                if s["is_qualified"]:
                    ...
                    st.markdown(f"**#{rank} {s['name']}**")
                    st.markdown(f"<span class='urgent-badge'>🔥 還差 {s['diff']} 人！</span>", unsafe_allow_html=True)
            with col2:
                progress_val = min(1.0, s["completed"] / s["target"])
                st.progress(progress_val)
                st.caption(f"進度：{s['completed']}/{s['target']} 人 ({s['rate']:.1f}%) | 門檻：{s['needed_60']} 人")
            with col3:
                if s["selected_seat"]:
...
def render_seat_map():
    st.subheader("🪑 活動室內座位即時配置圖")
    st.markdown("--- 舞台前方 (STAGE) ---")
        ...
        with cols[idx]:
            if s_val["status"] == "taken":
                ...
                <div class="seat-box seat-taken">
                    <div>🔴 {s_key}</div>
                    <div style="font-size:0.8rem; margin-top:4px;">{s_val['taken_by']}</div>
                ...
                <div class="seat-box seat-available">
                    <div>🟢 {s_key}</div>
                    <div style="font-size:0.8rem; margin-top:4px;">容量 {s_val['capacity']}人</div>
                    <div style="font-size:0.75rem;">(可選擇)</div>
                </div>
                """, unsafe_allow_html=True)
    st.caption("🟢 綠色：目前可搶選 | 🔴 紅色：已被其他院所鎖定")
...
def render_quiz_engine():
    ...
    if not u["logged_in"]:
        ...
        with st.form("login_form"):
            ...
            submitted = st.form_submit_button("進入搶位戰場")
            if submitted:
                if emp_id.strip():
                    u["logged_in"] = True
    ...
    st.markdown(f"#### 👋 同仁 `{u['emp_id']}` 歡迎上線！所屬：**{c_info['name']}**")
    
    all_done = all(u["progress"].values()) or (u["emp_id"] in st.session_state.completed_employees)
    
    if all_done:
        st.success(f"🎉 您已全數完成三大關卡！已為 **{c_info['name']}** 貢獻戰力！")
        if c_info["is_qualified"]:
            st.info("🏆 貴院所已成功達標 60%！目前排定選位順位，敬請鎖定現場即時看板！")
        else:
            st.warning(f"🔥 距離 60% 門檻還差 **{c_info['diff']}** 人，快呼叫院所夥伴一起挑戰！")
        return

    p_col1, p_col2, p_col3 = st.columns(3)
    p_col1.metric("① 家庭日題庫", "✅ 通關" if u["progress"]["family_day"] else "⬜ 挑戰中")
    p_col2.metric("② 馬光文化題庫", "✅ 通關" if u["progress"]["ma_kwang"] else "⬜ 挑戰中")
    p_col3.metric("③ 近期政策題庫", "✅ 通關" if u["progress"]["policy"] else "⬜ 挑戰中")

    if not u["progress"]["family_day"]:
        active_cat = "family_day"
        cat_title = "關卡一：家庭日活動通"
    elif not u["progress"]["ma_kwang"]:
        active_cat = "ma_kwang"
        cat_title = "關卡二：馬光核心文化"
    else:
        active_cat = "policy"
        cat_title = "關卡三：重要政策與規範"

    st.markdown(f"### 📍 當前挑戰：{cat_title}")
