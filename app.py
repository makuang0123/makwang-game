...
def load_employee_directory():
    try:
        # 如果上傳了 Excel 則即時讀取最新 Excel
        df = pd.read_excel("Employee-20260901200132.xlsx")
        ...
        for _, row in df.iterrows():
            db[row['員工編號']] = (row['所屬院所'], row['bday_mmdd'], str(row.get('職稱', '同仁')))
        return db
    except Exception:
        pass
